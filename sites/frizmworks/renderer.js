			...
			window.FNS.getDetailLinks = function( cbFunction ){
				...
				for(;z<zLen;++z)
				{
					...
					for(;i<iLen;++i){
						...
						r[ id ].nm = io.children[2].children[0].innerText.replace("상품명 : ","");
						r[ id ].salePrice = -1;
						r[ id ].msrp = -1;
						r[ id ].saleRatio = -1;
						r[ id ].isSoldOut = 0;
						...
						//상품이아닌 룩북을 올려놓은제품은 추가하지 않는다;
						if( r[ id ].nm.indexOf( "Lookbook") != -1 )
						{
							delete r[ id ];
							continue;
						}
						...
						if( io.children[2].children[1].childElementCount == 1 )
						{
							try
							{
								r[ id ].msrp = Number( io.children[2].children[1].children[0].children[1].innerText.replace( "KRW","" ).replace( /\,/gi,"" ).trim() );	
								r[ id ].salePrice = Number( io.children[2].children[1].children[0].children[1].innerText.replace( "KRW","" ).replace( /\,/gi,"" ).trim() );		
							}
							catch(er)
							{
								//debugger;
						...
						if( io.children[2].children[1].childElementCount == 2 )
						{
							try
							{
								r[ id ].msrp = Number( io.children[2].children[1].children[0].children[1].innerText.replace( "KRW","" ).replace( /\,/gi,"" ).trim() );

								io.children[2].children[1].children[1].children[1].children[0].innerText = ""
								r[ id ].salePrice = Number( io.children[2].children[1].children[1].children[1].innerText.replace( "KRW","" ).replace( /\,/gi,"" ).trim() );		
							}
							catch(er)
							{
								debugger;
						...
						{
							var isSoldOut = 0
							if( !io.children[2].children[2].children[0].children[0].classList.contains( "displaynone" ) )
							{
								isSoldOut = 1;
							}
