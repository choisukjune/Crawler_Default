	...
	onload = function(){
		...
		webview.addEventListener('did-finish-load', () => {
			...
			window.FNS.init = function(){
				...
				window.detailList = [];
				window.resultFileList = {};
				window.siteNm = "slowsteadyclub"
				window.siteUrl = "http://slowsteadyclub.com"
				//window.pageBaseUrl = "http://sculpstore.com/product/sale.html?cate_no=256&page="
				window.pageBaseUrls = [
					  "http://slowsteadyclub.com/product/list.html?cate_no=24&page="
					, "http://slowsteadyclub.com/product/list.html?cate_no=43&page="
				]
			...
			window.FNS.getMaxPage = function( cbFunction ){
				...
				webview.executeJavaScript(`
					var _el = window.document.getElementsByClassName("xans-element- xans-product xans-product-normalpaging")[0].lastElementChild.children[0].href
					Promise.resolve( _el )
				`
				).then(function(data){
			...
			window.FNS.downloadHtml = function( cbFunction ){
				...
				webview.loadURL( url );
				webview.executeJavaScript(`
					var _el = window.document.getElementsByClassName("prdList column3")[0].innerHTML
					Promise.resolve( _el )
				`
				).then(function(data){
			...
			window.FNS.getDetailLinks = function( cbFunction ){
				...
				for(;z<zLen;++z)
				{
					...
					for(;i<iLen;++i){
						...
						r[ id ].brand = io.children[0].children[1].children[0].innerText.replace( "| ", "" ).replace( " |", "" );
				
						r[ id ].nm = io.children[0].children[1].children[1].children[0].innerText;
						r[ id ].salePrice = -1;
						r[ id ].msrp = -1;
						r[ id ].saleRatio = -1;
						r[ id ].isSoldOut = 0;
						...
						//debugger;
						r[ id ].salePrice = Number( io.children[0].children[1].children[4].children[0].innerText.replace( /\,/gi,"" ).replace( "￦ ","" ).trim() );
						if( io.children[0].children[1].children[4].childElementCount == 1 ) r[ id ].msrp = r[ id ].salePrice;
						else r[ id ].msrp = Number( io.children[0].children[1].children[4].children[1].children[0].innerText.replace( /\,/gi,"" ).replace( "￦ ","" ).trim() );
						

						/*
						//사용하지않는 속성들임;
						var infos = io.children[2].children[2].children;

						var k = 0,kLen = infos.length,ko;
						for(;k<kLen;++k){
							
							ko = infos[ k ];

							if( ko.innerText.indexOf( "상품간략설명" ) != -1 )
							{
								r[ id ].info = ko.children[1].innerText.split( "\n" )
...
//						r[ id ].info.push( io.children[0].children[1].children[2].innerText.replace("(","").replace(")","").trim() );
						...
						{
							var isSoldOut = 0
							if( io.children[0].children[1].children[3].innerText != "" )
							{
								isSoldOut = 1;
							}
