	...
	onload = function(){
		...
		webview.addEventListener('did-finish-load', () => {
			...
			window.FNS.init = function(){
				...
				window.detailList = [];
				window.resultFileList = {};
				window.siteNm = "sculpstore"
				window.siteUrl = "http://sculpstore.com"
				//window.pageBaseUrl = "http://sculpstore.com/product/sale.html?cate_no=256&page="
				window.pageBaseUrls = [
					  "http://sculpstore.com/product/sale.html?cate_no=256&page="
					, "http://sculpstore.com/category/%EC%83%B5/255/?page="
				]
			...
			window.FNS.getDetailLinks = function( cbFunction ){
				...
				for(;z<zLen;++z)
				{
					...
					for(;i<iLen;++i){
						...
						r[ id ].img = io.children[1].children[0].children[0].src.replace( "file", "http" )
						
						if( r[ id ].img == "https://img.echosting.cafe24.com/thumb/img_product_medium.gif" )
						{
							r[ id ].img = "/web/upload/img_not_found.png"
						}
						
						r[ id ].brand = io.children[2].children[0].children[0].innerText;
				
						r[ id ].nm = io.children[2].children[0].children[1].innerText.replace("상품명 : ","");
						r[ id ].salePrice = -1;
						r[ id ].msrp = -1;
						r[ id ].saleRatio = -1;
						r[ id ].isSoldOut = 0;
						...
						r[ id ].msrp = Number( io.children[2].children[1].children[0].children[0].innerText.replace( /\,/gi,"" ) );
						r[ id ].salePrice = Number( io.children[2].children[1].children[0].children[1].innerText.replace( /\,/gi,"" ) );
						if( r[ id ].msrp == "" ) r[ id ].msrp = r[ id ].salePrice;
						...
						*/
						...
						{
							var isSoldOut = 0
							if( io.children[2].children[1].lastElementChild.innerText == "sold out" )
							{
								isSoldOut = 1;
							}
