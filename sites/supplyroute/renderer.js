	...
	onload = function(){
		...
		webview.addEventListener('did-finish-load', () => {
			...
			window.FNS.init = function(){
				...
				window.detailList = [];
				window.resultFileList = {};
				window.siteNm = "supplyroute"
				window.siteUrl = "https://supplyroute.co.kr"
				//window.pageBaseUrl = "http://sculpstore.com/product/sale.html?cate_no=256&page="
				window.pageBaseUrls = [
					  "https://supplyroute.co.kr/product/list.html?cate_no=722&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=723&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=739&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=746&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=755&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=759&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=772&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=778&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=750&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=777&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=808&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=787&page="
					, "https://supplyroute.co.kr/product/list.html?cate_no=795&page="
				]
			...
			window.FNS.getMaxPage = function( cbFunction ){
				...
				webview.executeJavaScript(`
					var _el = window.document.getElementsByClassName("xans-element- xans-product xans-product-normalpaging ec-base-paginate")[0].lastElementChild.href
					Promise.resolve( _el )
				`
				).then(function(data){

					if( window.UTIL.URL.paramToObject( data ).page == "#none" ) var maxPage = 1
					else var maxPage = window.UTIL.URL.paramToObject( data ).page * 1
			...
			window.FNS.getDetailLinks = function( cbFunction ){
				...
				for(;z<zLen;++z)
				{
					...
					var el = window.document.getElementById("_tmp").children[0].children;
					var i = 0, iLen = el.length, io;
					for(;i<iLen;++i){
						io = el[ i ];

						var href = io.children[0].children[4].href;
						...
						r[ id ].img = io.children[0].children[4].children[0].children[0].src.replace( "file", "http" )
						
						r[ id ].brand = "";
				
						r[ id ].nm = io.children[1].children[1].children[0].innerText.replace( "상품명 : ", "" ).replace( "[", "" ).replace( "]", " " ).replace( "(", "" ).replace( ")", " " ).replace( " - ", " " );
						...
						var _ta = io.children[1].children[3].children;
						var e = 0,eLen = _ta.length,eo;
						for(;e<eLen;++e){
							eo = _ta[ e ];
							if( eo.innerText.indexOf( "브랜드 : " ) != -1 )
							{
								r[ id ].brand = eo.innerText.replace( "브랜드 : ", "" )
							}

							if( eo.innerText.indexOf( "판매가 : " ) != -1 && eo.innerText.indexOf( "할인판매가 : " ) == -1 )
							{
								r[ id ].msrp = Number( eo.innerText.replace( "판매가 : ", "" ).replace( "원", "" ).replace( /\,/gi,"" ).trim() );
								if( isNaN( r[ id ].msrp ) ) debugger;
							}
						...
						if( r[ id ].salePrice == -1 ) r[ id ].salePrice = r[ id ].msrp;
						...
						{
							var isSoldOut = 0
							if( io.children[1].children[4].children[0].childElementCount != 0 && io.children[1].children[4].children[0].children[0].alt == "품절" )
							{
								isSoldOut = 1;
							}
