...
global.CONST.MongoDB.OPTIONS = {
	"self" : { ID : "admin", PWD : "tjrwns2482", HOST : "ec2-3-34-130-3.ap-northeast-2.compute.amazonaws.com", PORT : 59320 }	
};
...
var makeDbJsInsert = function( nm ){
    console.log( nm )
    var FILE_PATH = process.cwd() + "/contents/" + nm + "/result/" + nm + ".json";
    var TDBJS_PATH = process.cwd() + "/query/tdbjs/insertContents.tdbjs";
    var DBJS_PATH = process.cwd() + "/query/dbjs/";
    var _q = fs.readFileSync( TDBJS_PATH ).toString();
    var _d = fs.readFileSync( FILE_PATH ).toString();
    var q = _q.replace( "<!=DB_NM=!>", nm )
            .replace( "<!=DATA=!>", _d );
