...
var makeDbJsInsert = function( nm ){
    var FILE_PATH = process.cwd() + "/contents/" + nm + "/json/" + nm + ".json";
...
var makeBrandList = function(){
    var r = [];
    var arr = fs.readdirSync( process.cwd() + "/contents/" );
...
//-------------------------;
var FN00 = function(){
    var sites = makeBrandList();
    var i = 0,iLen = sites.length,io;
    for(;i<iLen;++i){
        io = sites[ i ]
        
        var _txt = `
		node tjrwns107308_exec_db_contents.js ${io}
        cmd/k
        `;
        
        fs.writeFileSync( "TJRWNS107308-INSERT DATA - " + io + ".bat", _txt,{ flag : "w" });
    }
}
