	...
	onload = function(){
		...
		webview.addEventListener('did-finish-load', () => {
			...
			window.FNS.init = function(){
				...
				window.resultFileList = {};
				window.siteNm = "GOCD"
				window.siteUrl = "https://cafe.naver.com/casuallydressed"
				window.pageBaseUrl = "https://cafe.naver.com/casuallydressed?iframe_url=/ArticleList.nhn%3Fsearch.clubid=19943558%26search.menuid=80%26search.boardtype=I%26search.totalCount=201%26search.page="
			}
			...
			window.FNS.downloadHtml = function( cbFunction ){
				...
				webview.executeJavaScript(`
					...
					new Promise((resolve, reject) => {
						//try
						//{
							var _el = window.document.body.innerHTML//.getElementById("cafe_main")
			...
			window.FNS.getDetailLinks = function( cbFunction ){
				...
				for(;z<zLen;++z)
				{
					...
					for(;i<iLen;++i){
						...
						var id = window.UTIL.URL.paramToObject( href ).articleid;

						r[ id ] = {};
						r[ id ].websiteNm = window.siteNm;
						r[ id ].url = window.siteUrl + href.replace( "file:///D:", "" )
						
						//r[ id ].img = io.children[0].children[0].src;
						//r[ id ].img = io.children[0].children[0].src.replace("https://cafethumb.pstatic.net","https://cafeptthumb-phinf.pstatic.net").replace("type=f200_200","type=w1600")
						r[ id ].img = io.children[0].children[0].getAttribute("onerror").split(",")[1].replace(" '","").replace("')","")
/*
https://cafethumb.pstatic.net          /MjAyMTAxMjlfMjkz/MDAxNjExOTI0OTcxNzE0.Xz_U9NbOGsDoE4SYk6N5iH1eLoikSY4aFok8zJHpyTcg.vKR2wT9XReqyIsNSwez-pe1jlxsU3HuPg_rhrH2BgCcg.JPEG/4939044B-63CA-47EC-885D-02D200F605C2.jpeg  ?  type=f200_200
https://cafeptthumb-phinf.pstatic.net  /MjAyMTAxMjlfMjkz/MDAxNjExOTI0OTcxNzE0.Xz_U9NbOGsDoE4SYk6N5iH1eLoikSY4aFok8zJHpyTcg.vKR2wT9XReqyIsNSwez-pe1jlxsU3HuPg_rhrH2BgCcg.JPEG/4939044B-63CA-47EC-885D-02D200F605C2.jpeg  ?  type=w1600


*/

						r[ id ].title = io.children[1].children[0].innerText;
						r[ id ].nick = io.children[1].children[1].innerText;
						try
						{
							var info = io.children[1].children[2].innerText.split(" 조회 ");
							r[ id ].date = info[0].trim();
							r[ id ].view = Number( info[1].trim().replace( /\,/gi,"") );
